...
export default function DirectPriceFix({
  formData,
  onFormDataChange,
  onPrev,
}: ReviewConfirmProps) {
  ...
  // Get fixed price (correct price) based on item name
  // This maps directly to the exact price in euros that should be displayed
  const getFixedPrice = (itemName: string): number => {
    switch(itemName) {
      ...
      case "Greek yogurt with honey": return 4.50;
      case "Fresh fruit platter": return 5.00;
      case "Premium Greek salad": return 12.00;
      case "Caesar salad": return 10.00;
      case "Quinoa salad": return 9.00;
      case "Caprese salad": return 11.00;
      case "Wild rocket salad": return 9.50;
      case "Beef carpaccio": return 15.00;
      case "Smoked salmon": return 14.00;
      case "Prosciutto and melon": return 13.00;
      case "Stuffed vine leaves": return 10.00;
      case "Greek spinach pie": return 8.00;
      case "Grilled lamb chops": return 22.00;
      case "Beef fillet": return 25.00;
      case "Grilled salmon": return 20.00;
      case "Roasted chicken": return 18.00;
      case "Moussaka": return 16.00;
      case "Smoked grilled eggplant with regional cheese": return 11.00;
      default: return 10.00; // Default price if item not found
    }
  };

  // Calculate subtotal using fixed prices
  const calculateSubtotal = (): number => {
    return formData.items.reduce((total, item) => {
      const itemName = getMenuItemName(item.menuItemId);
      const itemPrice = getFixedPrice(itemName);
  ...
  // Format display price with euro symbol
  ...
  const handlePaymentOption = (option: 'card' | 'invoice') => {
    if (option === 'card') {
      navigate(`/payment?orderId=new&amount=${calculateTotal() * 100}`);
    } else {
      toast({
