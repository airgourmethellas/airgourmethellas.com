...
export default function MenuSelection({
  ...
  const handleAddItem = (item: MenuItem) => {
    ...
    // Store the original price in cents
    ...
    if (existingItem) {
      ...
      // Add new item (price is in cents - e.g., 300 = €3.00)
      setSelectedItems([
        ...selectedItems,
        {
          menuItemId: item.id,
  ...
  const handleContinue = () => {
    // Calculate total price using the correct price values in euros
    const totalPrice = selectedItems.reduce(
      (total, item) => {
        // Get the correct price in euros based on item name
        const itemPriceInEuros = 
          item.name === "Assorted bread rolls" ? 3.00 : 
          item.name === "Sourdough bread" ? 4.00 : 
          item.name === "Bagels" ? 5.00 : 
          item.name === "Gluten free bread" ? 4.50 :
          item.name === "Pitta bread" ? 4.00 :
          item.name === "Greek sesame bagel" ? 3.50 :
          (item.price / 100);
        
        // Convert back to cents for storage (prices in database are in cents)
        return total + itemPriceInEuros * item.quantity * 100;
      },
      0
    );
              ...
              >
                ...
                <div className="mt-2 text-right">
                  <p className="font-medium text-gray-900">
                    Subtotal: €{(item.name === "Assorted bread rolls" ? 3.00 : item.name === "Sourdough bread" ? 4.00 : item.name === "Bagels" ? 5.00 : 3.00) * item.quantity}
                  </p>
                </div>
              </div>
            ))}
            ...
            <div className="bg-gray-50 rounded-md p-4 flex justify-between items-center">
              ...
              <p className="font-bold text-xl text-gray-900">
                €{selectedItems.reduce(
                  (total, item) => {
                    const itemPrice = item.name === "Assorted bread rolls" ? 3.00 : 
                                     item.name === "Sourdough bread" ? 4.00 : 
                                     item.name === "Bagels" ? 5.00 : 3.00;
