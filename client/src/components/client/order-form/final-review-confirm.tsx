...
import { getCorrectItemPrice, formatPriceDisplay } from "@/utils/fixed-price-formatter";
...
export default function FinalReviewConfirm({
  formData,
  onFormDataChange,
  onPrev,
}: ReviewConfirmProps) {
  ...
  // Get the correct price for a menu item
  const getMenuItemPrice = (id: number): number => {
    const itemName = getMenuItemName(id);
    return getCorrectItemPrice(itemName);
  };

  // Calculate subtotal using correct prices
  ...
  const handlePaymentOption = (option: 'card' | 'invoice') => {
    if (option === 'card') {
      // Convert total to cents for the payment system
        ...
        <>
              ...
              <AccordionContent>
                <div className="space-y-2">
                      ...
                      >
                        <div className="flex flex-col">
                          ...
                          <span className="text-sm text-gray-500">
                            {item.quantity} x {formatPriceDisplay(itemPrice)}
                          </span>
                        ...
                        <span className="font-medium">
                          {formatPriceDisplay(itemPrice * item.quantity)}
                        </span>
                      </div>
                    );
                  })}
                  ...
                  <div className="flex justify-between pt-2 border-t border-gray-200 font-medium">
                    <span>Subtotal</span>
                    <span>{formatPriceDisplay(calculateSubtotal())}</span>
                  </div>
                </div>
              </AccordionContent>
            </AccordionItem>
              ...
              <AccordionContent>
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between pt-2 text-base font-medium">
                    ...
                    <span>{formatPriceDisplay(150.00)}</span>
                  </div>
                </div>
              </AccordionContent>
            </AccordionItem>
          ...
          <div className="bg-gray-50 rounded-lg p-4 mt-6">
            <div className="flex justify-between items-center mb-2">
              ...
              <span className="text-base font-medium">{formatPriceDisplay(calculateSubtotal())}</span>
            </div>
            <div className="flex justify-between items-center mb-2">
              <span className="text-base font-medium">Delivery Fee:</span>
              <span className="text-base font-medium">{formatPriceDisplay(150.00)}</span>
            </div>
            <div className="flex justify-between items-center text-lg font-semibold border-t border-gray-200 pt-2 mt-2">
              <span>Total:</span>
              <span>{formatPriceDisplay(calculateTotal())}</span>
            </div>
          </div>
