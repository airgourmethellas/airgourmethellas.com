...
export default function DivisionFixReview({
  formData,
  onFormDataChange,
  onPrev,
}: ReviewConfirmProps) {
  ...
  // Using our central price formatter utility instead of local function

  // Sum all cents first
  const calculateSubtotalCents = (): number => {
    return formData.items.reduce((total, item) => {
      return total + (item.price * item.quantity);
    }, 0);
  };
  
  // Convert subtotal to euros for display
  const calculateSubtotal = (): number => {
    return calculateSubtotalCents() / 100;
  };

  // Delivery fee in cents (€150.00 = 15000 cents)
  const deliveryFeeCents = 15000;
  
  // Delivery fee in euros for display
  const deliveryFeeEuros = deliveryFeeCents / 100;
  
  // Calculate total in cents first
  const calculateTotalCents = (): number => {
    return calculateSubtotalCents() + deliveryFeeCents;
  };

  // Convert total to euros for display
  ...
  const handlePaymentOption = (option: 'card' | 'invoice') => {
    if (option === 'card') {
      // Use the amount already in cents for Stripe
        ...
        <>
          ...
          <div className="bg-gray-50 rounded-lg p-4 mt-6">
            <div className="flex justify-between items-center mb-2">
              ...
              <span className="text-base font-medium">€{formatPrice(calculateSubtotalCents())}</span>
            </div>
            <div className="flex justify-between items-center mb-2">
              <span className="text-base font-medium">Delivery Fee:</span>
              <span className="text-base font-medium">€{formatPrice(deliveryFeeCents)}</span>
            </div>
            <div className="flex justify-between items-center text-lg font-semibold border-t border-gray-200 pt-2 mt-2">
              <span>Total:</span>
              <span>€{formatPrice(calculateTotalCents())}</span>
            </div>
          </div>
