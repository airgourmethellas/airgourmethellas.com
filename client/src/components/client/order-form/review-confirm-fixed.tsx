...
// Hard-coded menu price map for accurate display
...
export default function ReviewConfirmFixed({
  formData,
  onFormDataChange,
  onPrev,
}: ReviewConfirmProps) {
  ...
  // When menu items data loads, update order items with proper prices
  useEffect(() => {
    if (menuItems && formData.items.length > 0) {
      const items = formData.items.map(item => {
        ...
        // Use our fixed price map for accurate display
        const correctPrice = MENU_PRICES[name] || (item.price / 100);
  ...
  const calculateTotal = (): number => {
    return calculateSubtotal() + 150.00; // Delivery fee is €150
  };
